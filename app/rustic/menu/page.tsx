...
export default function RusticMenu() {
      ...
      return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-10">
          {currentItems.map((item, index) => (
            <div key={index} className="flex justify-between items-start border-b border-[#E8E0D5] pb-4">
              <div className="pr-4">
                <h3 className="text-xl font-medium text-[#4A332A]">{item.name[lang]}</h3>
                {item.description && <p className="text-[#7D6B5D] text-sm mt-2 italic">{item.description[lang]}</p>}
              </div>
              <span className="text-lg font-bold text-[#C07F67] whitespace-nowrap">{item.price}</span>
            </div>
          ))}
        </div>
      );
    ...
    return (
      <div className="space-y-12">
        {Object.entries(groupedItems).map(([subcat, items], idx) => (
          <div key={idx}>
            <h3 className="text-md font-bold uppercase tracking-[0.15em] text-[#C07F67] mb-6 border-b border-[#C07F67] pb-2 inline-block">
              {subcat}
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-6">
              {items.map((item, index) => (
                <div key={index} className="flex justify-between items-start border-b border-[#E8E0D5] pb-3">
                  <div className="pr-4">
                    <h4 className="text-lg font-medium text-[#4A332A]">{item.name[lang]}</h4>
                    {item.description && <p className="text-[#7D6B5D] text-xs mt-1 italic">{item.description[lang]}</p>}
                  </div>
                  <span className="text-md font-bold text-[#C07F67] whitespace-nowrap">{item.price}</span>
                </div>
              ))}
            </div>
          </div>
  ...
  const pageVariants: Variants = {
    ...
    exit: (dir: number) => ({ opacity: 0, x: dir > 0 ? -20 : 20, transition: { duration: 0.4, ease: "easeIn" } })
  };
  ...
  return (
    <main className="min-h-screen bg-[#F9F6F0] text-[#5C4033] p-6 md:p-12 font-serif relative">
      <div className="max-w-5xl mx-auto">
        ...
        <header className="flex flex-col md:flex-row justify-between items-center mb-10 pb-6 border-b border-[#E8E0D5]">
        ...
        <div className="flex flex-wrap gap-3 mb-12 pb-4 font-sans">
          {categories.map((category) => (
            <button
              key={category.id}
              onClick={() => handleCategoryChange(category.id)}
              className={`px-6 py-2 rounded-full uppercase tracking-wider text-xs font-bold transition-all ${
                activeCategory === category.id 
                  ? 'bg-[#C07F67] text-[#F9F6F0] shadow-md' 
                  : 'bg-transparent text-[#7D6B5D] border border-[#E8E0D5] hover:border-[#C07F67]'
              }`}
            >
        ...
        <div className="bg-white rounded-2xl p-8 md:p-12 shadow-sm border border-[#E8E0D5] min-h-[50vh]">
          <AnimatePresence mode="wait" custom={direction}>
            <motion.div key={activeCategory} custom={direction} variants={pageVariants} initial="initial" animate="animate" exit="exit">
              {renderItems()}
            </motion.div>
          </AnimatePresence>
        </div>
