...
export default function UnifiedHomePage() {
  ...
  return (
    <main className="flex flex-col min-h-screen w-full bg-white">
      ...
      {/* ================= HERO SECTION ================= */}
      {/* Updated min-h-[100svh] ensures the hero perfectly fits the viewport, even with expanding mobile address bars */}
      <section className="flex flex-col lg:flex-row min-h-[100svh] w-full bg-[#353941]">
        
        {/* Left Panel - Branding */}
        {/* Increased vertical padding on mobile (py-24) to accommodate safe areas */}
        <div className="w-full lg:w-1/2 flex flex-col items-center justify-center py-24 px-6 lg:p-12 text-center relative z-10 flex-grow">
          
          <motion.div 
            initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, ease: "easeOut" }}
            className="relative flex flex-col items-center justify-center mt-[-2rem] lg:mt-[-4rem]"
          >
            <div className="absolute w-40 h-40 md:w-48 md:h-48 border-[1px] border-[#cda1b1]/40 rounded-full -top-4 md:-top-6 left-1/2 -translate-x-1/2 z-0"></div>
            <div className="absolute w-40 h-40 md:w-48 md:h-48 border-[1px] border-[#cda1b1]/40 rounded-full -top-2 md:-top-4 left-1/2 -translate-x-[40%] z-0"></div>
          ...
          {/* Adjusted touch target height for mobile */}
          <motion.button
          ...
          {/* Increased touch target hit area */}
        ...
        {/* Changed h-[50vh] to min-h-[50vh] so content never overlaps if browser viewport shrinks */}
        <div className="w-full lg:w-1/2 min-h-[50vh] lg:min-h-[100svh] grid grid-cols-2 grid-rows-2 bg-white">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }} className="relative w-full h-full border-r-[2px] border-b-[2px] md:border-r-[4px] md:border-b-[4px] border-white overflow-hidden group">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 0.1 }} className="relative w-full h-full border-b-[2px] md:border-b-[4px] border-white overflow-hidden group">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 0.2 }} className="relative w-full h-full border-r-[2px] md:border-r-[4px] border-white overflow-hidden group">
      ...
      <section id="menu-section" className="min-h-screen bg-[#faf8f5] px-4 md:px-6 py-16 md:py-24 relative overflow-hidden">
        <div className="max-w-4xl mx-auto relative z-10">
          ...
          {/* Added horizontal scroll (overflow-x-auto) and invisible scrollbar class */}
          <div className="flex justify-start lg:justify-center gap-2 mb-10 md:mb-16 overflow-x-auto hide-scrollbar pb-4 snap-x snap-mandatory">
            {menuCategories.map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`snap-center flex-shrink-0 relative px-4 md:px-5 py-2.5 text-[10px] md:text-xs font-bold tracking-widest uppercase transition-colors ${
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="relative bg-white border-[1px] border-[#cda1b1]/30 rounded-3xl p-6 md:p-16 shadow-sm min-h-[400px]"
            >
              
              {/* Randomized Floating Corner Image (Hidden on strict mobile to maximize reading space) */}
              {randomImage && (
                <motion.div 
